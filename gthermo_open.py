# Copyright (C) 2018 Pinwen Guan
# All rights reserved.
#
# This file is part of depye, a python code for EOS fitting and calculations of thermodynamics properties based on the Debye model written# by Pinwen Guan at Carnegie Mellon University in Nov, 2018.
...
def read_expt(expt='expt'):
	fexpt=open(expt)
	cexpt=[i.replace('\n',' ').replace('\t',' ').strip(' ') for i in fexpt.readlines() if not '#' in i]
...
"""EOS fitting based on ASE. Return equilibrium volume, energy and bulk modulus in angstrom-eV unit"""
...
def debye_single(V,E,N,M,s=0.617,Tmax=1000,nat=1):
	...
	return [V0,E0,B0/1e9,BP0],EVfit,[T,G,VT,BT,S,H,TEC,Cp,Cv]

"""calculate, plot and save thermodynamic properties based on the Debye model"""

def debye(ev='e-v.dat',trimve='F',expt='expt',struc='POSCAR',out='Debye-thermo',s=0.617,poisson='poisson',Tmax=1000,Tstep_write=10,show='T',write='T',nat=0):
	hbar=1.054571800e-34;kB=1.38064852e-23;q=1.60217662e-19;NA=6.022140e23
	A=(6*np.pi**2)**(1/3)*hbar/kB
	...
	if len(cname) != 0:
		...
		cna=[str(i) for i in range(len(cev[-1]))]
	...
	[V0,E0,B0,BP0]=[[1 for i in range(len(cev[0])-1)] for j in range(4)]
	EVfit=[[] for i in range(len(cev[0])-1)]
	[T,G,VT,BT,S,H,TEC,Cp,Cv]=[[[] for i in range(len(cev[0])-1)] for j in range(9)]
	for i in range(len(cev[0])-1):
		ds=debye_single(V,E[i],N,M,s,Tmax,nat=nat)
		if trimve=='T':
			Vtrim,E[i]=trim_ve(V,E[i])
			ds=debye_single(Vtrim,E[i],N,M,s,Tmax,nat=nat)
		[V0[i],E0[i],B0[i],BP0[i]]=ds[0]
		EVfit[i]=ds[1]
		[T[i],G[i],VT[i],BT[i],S[i],H[i],TEC[i],Cp[i],Cv[i]]=ds[2]
	...
	if show=='T':
		...
		plt.legend()
		k=0
		property_names=['G (kJ/mol)','V (cm^3/mol)','B (GPa)','S (J/mol/K)','H (kJ/mol)','TEC (1e-6/K)','Cp (J/mol/K)','Cv (J/mol/K)']
		property_symbols=['G','V','B','S','H','TEC','Cp','Cv']
		for i in [G,VT,BT,S,H,TEC,Cp,Cv]:
			plt.subplot(342+k)
			...
			if os.path.exists(expt):
				for j in range(len(data)):
					if data[j]['property']==property_symbols[k]:
						plt.scatter(data[j]['T'],data[j]['value'],color='orangered',marker=markers[int(data[j]['ref'])],label='Expt.['+data[j]['ref']+']')
			plt.xlabel("T (K)")
			plt.ylabel(property_names[k])
			plt.legend()
			k=k+1
